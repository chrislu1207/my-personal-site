...
> = {
  3: { title: 'Easy', size: 3, colorAdjustment: 50, gridSize: 'grid-cols-3' },
  4: { title: 'Medium', size: 4, colorAdjustment: 40, gridSize: 'grid-cols-4' },
  5: { title: 'Hard', size: 5, colorAdjustment: 30, gridSize: 'grid-cols-5' },
  6: {
    title: 'Extreme',
    size: 6,
    colorAdjustment: 20,
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16">
      <main className="flex flex-col gap-[32px] row-start-2 items-start">
        <h1 className="text-2xl font-bold">Colour.io</h1>
        <div className="flex flex-row gap-4 items-center">
          <span>Select Difficulty:</span>
          {Object.entries(DifficultyConfig).map(([key, config]) => (
            <button
              key={key}
              className="rounded-full p-3 border border-white hover:border-fuchsia-500 disabled:border-fuchsia-500 hover:cursor-pointer"
        ...
        <div className="flex items-center justify-items-center text-center">
          <div className="mr-1">Consecutive Wins:</div>
          <div className="font-bold text-3xl">{wins}</div>
        </div>
        <div
          className={`size-160 grid ${DifficultyConfig[difficulty].gridSize} gap-2`}
        >
          {gridItems.map((item) => (
            <div
